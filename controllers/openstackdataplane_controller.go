...
func (r *OpenStackDataPlaneReconciler) GetLogger(ctx context.Context) logr.Logger {
	return log.FromContext(ctx).WithName("Controllers").WithName("OpenStackDataPlane")
}
...
func (r *OpenStackDataPlaneReconciler) Reconcile(ctx context.Context, req ctrl.Request) (result ctrl.Result, _err error) {
	...
	if err != nil {
		...
		Log.Error(err, fmt.Sprintf("unable to acquire helper for OpenStackDataPlane %s", instance.Name))
		return ctrl.Result{}, err
	}
	...
	ctrlResult, err := createOrPatchDataPlaneResources(ctx, instance, r, helper)
	if err != nil {
		return ctrl.Result{}, err
	...
	if instance.Spec.DeployStrategy.Deploy {
		Log.Info("Starting DataPlane deploy")
		...
		Log.Info("found roles", "total", len(roles.Items))
		if len(instance.Spec.Roles) > len(roles.Items) {
			shouldRequeue = true
		}

		for _, role := range roles.Items {
			Log.Info("DataPlane deploy", "role.Name", role.Name)
			...
			Log.Info("Role", "DeployStrategy.Deploy", role.Spec.DeployStrategy.Deploy, "Role.Namespace", instance.Namespace, "Role.Name", role.Name)
			if !role.Spec.DeployStrategy.Deploy {
				_, err := controllerutil.CreateOrPatch(ctx, helper.GetClient(), &role, func() error {
					Log.Info("Reconciling Role", "Role.Namespace", instance.Namespace, "Role.Name", role.Name)
			...
			if !role.IsReady() {
				Log.Info("Role", "IsReady", role.IsReady(), "Role.Namespace", instance.Namespace, "Role.Name", role.Name)
				shouldRequeue = true
				mirroredCondition := role.Status.Conditions.Mirror(condition.ReadyCondition)
				if mirroredCondition != nil {
					Log.Info("Role", "Status", mirroredCondition.Message, "Role.Namespace", instance.Namespace, "Role.Name", role.Name)
	...
	if shouldRequeue {
		Log.Info("one or more roles aren't ready, requeueing")
...
func createOrPatchDataPlaneResources(ctx context.Context, instance *dataplanev1.OpenStackDataPlane, r *OpenStackDataPlaneReconciler, helper *helper.Helper) (ctrl.Result, error) {
	// create DataPlaneRoles
	roleManagedHostMap := make(map[string]map[string]baremetalv1.InstanceSpec)
	err := createOrPatchDataPlaneRoles(ctx, instance, r, helper, roleManagedHostMap)
	if err != nil {
	...
	err = createOrPatchDataPlaneNodes(ctx, instance, r, helper)
	if err != nil {
		instance.Status.Conditions.MarkFalse(
			condition.ReadyCondition,
			condition.ErrorReason,
...
func createOrPatchDataPlaneNodes(ctx context.Context, instance *dataplanev1.OpenStackDataPlane, r *OpenStackDataPlaneReconciler, helper *helper.Helper) error {
	Log := r.GetLogger(ctx)
	client := helper.GetClient()

	for nodeName, nodeSpec := range instance.Spec.Nodes {
		Log.Info("CreateDataPlaneNode", "nodeName", nodeName)
...
func createOrPatchDataPlaneRoles(ctx context.Context,
	instance *dataplanev1.OpenStackDataPlane, r *OpenStackDataPlaneReconciler, helper *helper.Helper,
	...
	for roleName, roleSpec := range instance.Spec.Roles {
		...
		Log.Info("Create Or Patch DataPlaneRole", "roleName", roleName)
