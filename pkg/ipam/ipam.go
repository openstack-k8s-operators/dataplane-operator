...
type AssignIPDetails struct {
	...
	RangeStart    net.IP
	RangeEnd      net.IP
	Reservelist   []dataplanev1beta1.IPReservation
...
func AssignIP(assignIPDetails AssignIPDetails, fixedIP net.IP) (dataplanev1beta1.IPReservation, []dataplanev1beta1.IPReservation, error) {
	...
	if fixedIP != nil {
		...
		return dataplanev1beta1.IPReservation{}, nil, err
	}
...
func fixedIPExists(assignIPDetails AssignIPDetails, ip net.IP) (bool, dataplanev1beta1.IPReservation, error) {
	...
	for i := iPToBigInt(firstip); iPToBigInt(lastip).Cmp(i) == 1 || iPToBigInt(lastip).Cmp(i) == 0; i.Add(i, big.NewInt(1)) {
		if iPToBigInt(ip).Cmp(i) == 0 {
			for _, rs := range assignIPDetails.Reservelist {
				if rs.IP == ip.String() {
					return false, dataplanev1beta1.IPReservation{}, fmt.Errorf("Already reserved %s", ip.String())
				}
			}
			return true, dataplanev1beta1.IPReservation{IP: ip.String(), VIP: false}, nil
		}
	}
	return false, dataplanev1beta1.IPReservation{}, fmt.Errorf("Does not exist in range %s", ip.String())
}

// IterateForAssignment iterates given an IP/IPNet and a list of reserved IPs
func iterateForAssignment(assignIPDetails AssignIPDetails) (dataplanev1beta1.IPReservation, []dataplanev1beta1.IPReservation, error) {

	reservedIP := dataplanev1beta1.IPReservation{}
	firstip := assignIPDetails.RangeStart
	var lastip net.IP
...
MAINITERATION:
	...
	if !performedassignment {
		return dataplanev1beta1.IPReservation{}, assignIPDetails.Reservelist, AssignmentError{firstip, lastip, assignIPDetails.IPNet}
	}
