...
import (
	...
	ansibleeev1beta1 "github.com/openstack-k8s-operators/openstack-ansibleee-operator/api/v1beta1"
...
func ConditionalDeploy(
	...
	if status.Conditions.IsFalse(readyCondition) {
		...
		if ansibleEE.Status.JobStatus == ansibleeev1beta1.JobStatusSucceeded {
			log.Info(fmt.Sprintf("Condition %s ready", readyCondition))
		...
		if ansibleEE.Status.JobStatus == ansibleeev1beta1.JobStatusRunning || ansibleEE.Status.JobStatus == ansibleeev1beta1.JobStatusPending {
		...
		if ansibleEE.Status.JobStatus == ansibleeev1beta1.JobStatusFailed {
